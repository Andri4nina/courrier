            ...
            <div class="bounceslideInFromRight p-5 min-w-4xl crud-card">
                <div class="top-card">
                    <div class=" text-white font-bold items-center flex gap-2">
                        ...
                        <h4 class="text-white my-5">Tous les Clients</h4>
                    </div>
                        ...
                        <form action="{{ route('client.index') }}">
                            <div class="flex justify-center items-center relative">
                                ...
                                <div class="w-10/12 inputfield px-4 py-2 my-5 border-b-2">
                                    <input type="text" placeholder="Chercher un client" name="search"
                                ...
                                @if (count($clients) > 0)
                                    @foreach ($clients as $clients)
                                        <tr class="hover:bg-slate-800 hover:text-slate-50   border-b">
                                            <td class="px-4 py-2  ">
                                                <a href={{ route('client.detail', $clients->id) }}> <span class="uppercase
                                                    font-semibold">{{ $clients->nom }} </span> {{ $clients->prenom }}
                                                </a>
                                            </td>
                                            <td class="px-4 py-2  ">
                                                {{ $clients->cin }}
                                            ...
                                            <td class="flex justify-center items-center my-5 gap-2 px-4 py-2">

                                                <form action="{{ route('client.edit', $clients->id) }}">
                                                    <button

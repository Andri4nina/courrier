...
<div class="w-10/12 mx-auto max-h-screen h-screen flex justify-center mt-40">
    <div class=" w-full">
        <h3 class="text-white text-3xl font-semibold  flex items-center">
            <a href={{ route('auth.toMenu') }} class="flex justify-center items-center"><i class="bx bx-chevron-left"></i></a>
            <p>Postes</p>
        </h3>
            ...
            <div class="  w-full">
                <div class=" flex justify-end ">
                    <form action="{{ route('poste.create') }}">
                        <button class="mt-5 bg-blue-800  hover:bg-blue-600 hover:shadow-blue-700 hover:shadow-lg px-4  py-2 rounded-sm text-white transition-all "><span>Nouvelle poste +</span></button>
                    </form>
                </div>

                <form action="{{ route('poste.index') }}">
                    <div class="flex justify-center items-center relative">
                        <button class=" w-2/12 bg-blue-800 h-auto  hover:bg-blue-600 hover:shadow-blue-700 hover:shadow-lg px-4  py-2 rounded-sm text-white transition-all ">
                            <span>Rechercher</span>
                        </button>
                        <div class="w-10/12 inputfield px-4 py-2 my-5 border-b-2">
                            <input type="text" placeholder="Chercher une poste" name="search" class="bg-transparent outline-none text-white">
                        </div>
                                ...
                                <tr class="hover:bg-slate-300 hover:text-slate-600  border-b">
                                    ...
                                    <td class="flex justify-center items-center gap-2 py-2 px-4">
                                        <form action="{{ route('poste.edit' ,$postes->id )}}">
                                            <button class="w-8 h-8 border text-blue-500 border-blue-500  hover:bg-blue-500 hover:shadow-blue-700 hover:shadow-lg hover:text-white  font-bold "><i class="bx bx-pencil"></i></button>
                                        </form>
                                        <form action="{{ route('poste.destroy' ,$postes->id )}}" method="POST">
                                            @method('delete')
                                            @csrf

                                            <button class="w-8 h-8 border text-red-500 border-red-500   hover:bg-red-500 hover:shadow-red-700 hover:shadow-lg hover:text-white font-bold "onclick="deleteConfirm(event)"><i class="bx bx-trash"></i></button>
                                        </form>
